...
namespace WetPicsTelegramBot.WebApp.Providers
{
    class MessagesProvider : IMessagesProvider
    {
        ...
        public ReplyMessage ChangeLogMessage
            => new ReplyMessage($"<b>2.18.0</b>{_nl}" +
                                $"* Бот переведен в архивный режим. Спасибо, за то, что были с нами все это время!{_nl}" +
                                $"Постинг новых изображений выключен и не будет более доступен. " +
                                $"Команды настройки постинга изображений остаются доступными, но ни на что не влияют. " +
                                $"Весь остальной функционал работает как раньше и будет работать еще некоторое время.{_nl}{_nl}" +
                                ...
                                $"<b>2.15.0</b>{_nl}" +
                                $"* Минимальный интервал постинга изменен на 15 минут.{_nl}{_nl}" +
                                ...
                                $"<b>2.8.0</b>{_nl}" +
                                $"* Добавлен пропуск чата, в который не удалось запостить следующее изображение.{_nl}{_nl}" +
        ...
        public ReplyMessage WetpicsIncorrectInterval
            => new ReplyMessage("Введен некорректный интервал (минимальный интервал 15 минут).");
