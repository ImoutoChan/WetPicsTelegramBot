    ...
    class DialogServive
    {
        ...
        private async void BotOnMessageReceived(object sender, MessageEventArgs messageEventArgs)
        {
            ...
            if (command == "/help" || command == "/start")
            {
                ...
                await _api.SendTextMessageAsync(message.Chat.Id, text, ParseMode.Markdown, replyToMessageId: message.MessageId);
            }
            else if (command == "/deactivatePhotoRepost")
            {
                await DbRepository.Instance.RemoveChatSettings((string) message.Chat.Id);

                await _api.SendTextMessageAsync(message.Chat.Id, "Пересылка фотографий отключена.", replyToMessageId: message.MessageId);
            }
            else if (command == "/activatePhotoRepost")
            {
                var text = "Введите Id канала/чата для репоста. Для корректной работы, бот должен быть администратором канала, либо должен состоять в выбранной группе." + Environment.NewLine +
                           "Пример Id: @channelName u00000000 с00000000 g00000000" + Environment.NewLine +
                           "Справка: /activatePhotoRepostHelp";

                await _api.SendTextMessageAsync(message.Chat.Id, text, replyMarkup: new ForceReply { Force = true }, replyToMessageId: message.MessageId);
            }
            else if (command == "/activatePhotoRepostHelp")
            {
                var text = "Id может начинаться с @ для публичных каналов/чатов с заданным username. " +
                           "Для определения Id приватных получателей перейдите в web клиент, выберете нужного получателя." + Environment.NewLine + Environment.NewLine +
                ...
                await _api.SendTextMessageAsync(message.Chat.Id, text, ParseMode.Html, replyToMessageId: message.MessageId);
            }
                ...
                if (message.ReplyToMessage?.Text.StartsWith("Введите Id к") ?? false)
                {
                    ...
                    switch (fl)
                    {
                        ...
                        case 'c':
                            ...
                            break;
                        default:
                            await _api.SendTextMessageAsync(message.Chat.Id, "Неверный формат Id", replyToMessageId: message.MessageId);
                            break;
                    }
                }
            }
